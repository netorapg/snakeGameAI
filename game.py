...
class SnakeGameAI:
    ...
    def play_step(self, action):
        ...
        prev_distance = self._distance_to_food(self.head)  # Distância antes de mover
    ...
    def _move(self, action):
        ...
        if np.array_equal(action, [1, 0, 0]):
            ...
            next_idx = (idx + 1) % 4
            new_dir = clock_wise[next_idx]  # Direita: R -> D -> L -> U
        else:  # [0, 0, 1]
            next_idx = (idx - 1) % 4
            new_dir = clock_wise[next_idx]  # Esquerda: R -> U -> L -> D
    ...
    def _distance_to_food(self, position):
        return abs(position.x - self.food.x) + abs(position.y - self.food.y)
